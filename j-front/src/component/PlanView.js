...
function Plan() {
  ...
  const fetchUserProfiles = async () => {
    try {
      ...
      setUserProfiles(response.data.data); // Assume the data is in `response.data.data`
    } catch (error) {
  ...
  const sendInvite = async () => {
    try {
      const response = await apiClient.post(`/plans/${plan.id}/invite`, {
        email: inviteEmail,
      }, {
        headers: { Authorization: `${localStorage.getItem('authToken')}` },
      });
      ...
      if (response.status === 200) {
        ...
        closeModal();  // Close the modal after sending the invite
      } else {
  ...
  // Define the openModal function here
  const openModal = () => setIsModalOpen(true);

  // Define the closeModal function here
  const closeModal = () => setIsModalOpen(false);

  useEffect(() => {
    fetchColumns();
    loadKakaoMapsScript();
    fetchUserProfiles(); // Fetch user profiles when component mounts
  }, []);
  ...
  return (
      <div style={styles.container}>
          ...
          {isEditing ? (
              <div>
                <input
                    ...
                    style={styles.input}
                />
                <button style={styles.button} onClick={handleTitleChange}>Save
                </button>
              ...
              <div>
                ...
                <div style={styles.plusButton} onClick={openModal}>+</div>
              </div>
          )}
        </div>
        ...
        <div style={styles.form}>
          ...
          <button style={{...styles.button, ...styles.formButton}}
                  onClick={handleAddColumn}>Add Column
          </button>
        </div>
        ...
        <div style={styles.form}>
          ...
          />
          <button style={{...styles.button, ...styles.formButton}}
                  onClick={handleAddCard}>Add Card
          </button>
        </div>
        ...
        {editingCardId && (
            <div style={{marginTop: '20px'}}>
              ...
              <button style={styles.button} onClick={handleSaveCardChanges}>Save
                Changes
              </button>
              <button style={styles.buttonSecondary}
                      onClick={() => setEditingCardId(null)}>Cancel
              </button>
            </div>
        )}
        {isModalOpen && (
            <>
              ...
              <div style={styles.modal}>
                ...
                {/* Add your modal form or content here */}
                <p>Invite Users to Join the Plan</p>
                <input
                    type="email"
                    value={inviteEmail}
                    ...
                    style={styles.input}
                />
                <button style={styles.button} onClick={sendInvite} >Invite</button>
              </div>
            </>
        )}
